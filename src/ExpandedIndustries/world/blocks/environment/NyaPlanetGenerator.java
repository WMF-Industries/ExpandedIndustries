...
public class NyaPlanetGenerator extends PlanetGenerator {
    ...
    Block[][]arr = {
            {Blocks.stone, Blocks.ice, Blocks.snow, Blocks.water},
            {Blocks.ice, Blocks.ice, Blocks.snow, Blocks.cryofluid},
            { Blocks.snow, Blocks.ice, Blocks.cryofluid}
    };
    ...
    protected void generate(){
        ...
        Vars.state.rules.waveSpacing = Mathf.lerp(60 * 65 * 2, 60f * 60f * 1f, Math.max(difficulty - waveTimeDec, 0f));
        Vars.state.rules.waves = sector.info.waves = true;
        Vars.state.rules.enemyCoreBuildRadius = 600f;

        //spawn air only when spawn is blocked
        Vars.state.rules.spawns = Waves.generate(difficulty, new Rand(sector.id), Vars.state.rules.attackMode, Vars.state.rules.attackMode && spawner.countGroundSpawns() == 0, naval);
    }
